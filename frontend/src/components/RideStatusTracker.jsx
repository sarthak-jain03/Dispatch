...
import { Search, UserCheck, MapPin, Play, CheckCircle2, XCircle, Car, Clock, DollarSign, AlertTriangle } from 'lucide-react';
...
const LIFECYCLE_STEPS = [
    ...
    { status: 'PAYMENT_COMPLETED', label: 'Completed', icon: CheckCircle2 },
];
...
export default function RideStatusTracker({ 
    ...
    const isMatchingFailed = ride.status === 'MATCHING_FAILED';
    const isPaymentFailed = ride.status === 'PAYMENT_FAILED';
    const isTerminalState = isCancelled || isMatchingFailed || isPaymentFailed || ride.status === 'PAYMENT_COMPLETED' || ride.status === 'COMPLETED';
        ...
        <div className="flex flex-col gap-6">
            ...
            {isMatchingFailed && (
                <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-2xl p-4">
                    ...
                    <div>
                        <p className="text-red-400 font-semibold">Matching Failed</p>
                        <p className="text-sm text-uber-gray-400">No driver could accept your request. Releasing locks.</p>
                    </div>
                </div>
            )}
            ...
            {isPaymentFailed && (
                <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-2xl p-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-uber-gray-400">Transaction failed. Please try again.</p>
                    </div>
                </div>
            )}
                        ...
                        return (
                            <div key={step.status} className="relative flex gap-4">
                                ...
                                {idx < LIFECYCLE_STEPS.length - 1 && (
                                    <div className={`absolute left-[17px] top-[36px] w-0.5 h-[calc(100%-4px)] ${isCompleted ? 'bg-uber-green' : 'bg-uber-gray-700'}`} />
                                )}
                                ...
                                <div className="pb-8 pt-1.5">
                                    ...
                                    {isActive && step.status === 'REQUESTED' && (
                                        <p className="text-xs text-uber-gray-400 mt-0.5">Looking for nearby drivers...</p>
                                    )}
                                    {isActive && step.status === 'DRIVER_ASSIGNED' && ride.driverId && (
                                    ...
                                    {isActive && step.status === 'PAYMENT_PENDING' && (
                                        <p className="text-xs text-uber-gray-400 mt-0.5">Please finalize payment below.</p>
                                    )}
                                </div>
                            </div>
                        );
            ...
            {/* Driver Info Card */}
            {ride.driverId && !isTerminalState && (
                <div className="bg-uber-gray-800 rounded-2xl p-4 border border-uber-gray-700 animate-[fade-in_0.3s_ease-out]">
            ...
            {!isTerminalState && (
                <div className="space-y-2 pt-2 border-t border-uber-gray-800">
                    <p className="text-xs text-uber-gray-500 uppercase tracking-wider font-semibold mb-3">
                        Demo Controls
                    </p>

                    {ride.status === 'DRIVER_ASSIGNED' && (
                        <button onClick={onDriverArrived} disabled={actionLoading} className="btn-secondary w-full">
                            📍 Mark Driver Arrived
                        </button>
                    )}

                    {ride.status === 'DRIVER_ARRIVED' && (
                        <button onClick={onStartRide} disabled={actionLoading} className="btn-green w-full">
                            ▶️ Start Ride
                        </button>
                    )}

                    {ride.status === 'IN_PROGRESS' && (
                        <button onClick={onCompleteRide} disabled={actionLoading} className="btn-green w-full">
                            🏁 Complete Ride
                        </button>
                    )}

                    {(ride.status === 'REQUESTED' || ride.status === 'DRIVER_ASSIGNED') && (
                        <button onClick={onCancelRide} disabled={actionLoading} className="btn-danger w-full mt-2">
                            Cancel Ride
                        </button>
                    )}
                </div>
